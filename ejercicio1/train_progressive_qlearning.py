...
def create_progressive_config():
    ...
    # Configuración inspirada en tu tabla - 5000 episodios por iteración
    configs = [
        # Iteraciones 1-3: Exploración inicial alta
        ...
        # Iteraciones 10-12: Reducción de exploración
        ...
        # Iteraciones 13-15: Convergencia
        ...
        # Iteraciones 16-18: Explotación final
...
def train_progressive_qlearning():
    """Entrena Q-learning con configuración progresiva"""
    
    print("🎯 ENTRENAMIENTO PROGRESIVO Q-LEARNING")
    print("="*60)
    print("📋 Configuración:")
    print("   • 18 iteraciones con hiperparámetros ajustables")
    print("   • 5,000 episodios por iteración")
    print("   • Total: 90,000 episodios")
    print("   • Tiempo estimado: ~3.6 horas")
    ...
    # Discretización balanceada: suficiente granularidad sin ser excesiva
    # [altitude, vz, target_altitude, runway_distance]
    state_bins = [6, 6, 8, 8]  # 2,304 estados - balance entre eficiencia y precisión
    
    agent = QLearningAgent(
        n_actions=n_actions,
        state_bins=state_bins,
        alpha=0.1,  # Se actualizará en cada iteración
        gamma=0.99,  # Se actualizará en cada iteración
        epsilon=1.0  # Se actualizará en cada iteración
    )
    ...
    for iteration, config in enumerate(configs, 1):
        ...
        # Capturar snapshot de Q-table para visualización
...
def save_results(agent, iteration_summaries, all_episode_rewards, 
    ...
    model_path = "models/progressive_qlearning.pkl"
    
    with open(model_path, 'wb') as f:
        pickle.dump({
            'algorithm': 'Progressive Q-Learning',
...
def create_visualizations(iteration_summaries, all_episode_rewards, q_table_evolution):
    """Crea visualizaciones del entrenamiento progresivo"""
    
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
    
    # Gráfico 1: Evolución de recompensas por iteración
    ...
    # Gráfico 2: Evolución de hiperparámetros
    ...
    # Gráfico 3: Curva de aprendizaje (ventana móvil)
    window_size = 500
    ...
    # Gráfico 4: Exploración de estados
    ...
    plt.tight_layout()
    plt.savefig('progressive_qlearning_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"📈 Gráficos guardados: progressive_qlearning_analysis.png")
    
    # Crear tabla resumen
    create_summary_table(iteration_summaries)

def create_summary_table(iteration_summaries):
    """Crea tabla resumen similar a la que mostraste"""
    
    print(f"\n📋 TABLA RESUMEN - ENTRENAMIENTO PROGRESIVO")
    print("="*100)
    ...
    for s in iteration_summaries:
        print(f"{s['iteration']:<4} | {s['episodes']:<8} | {s['alpha']:<5} | "
              ...
              f"{len(iteration_summaries):<8}")  # Placeholder para estados
    
    print("-"*100)
    print(f"Total: {sum(s['episodes'] for s in iteration_summaries):,} episodios (90,000 episodios esperados)")

def train_short_qlearning():
    """Entrenamiento corto para validación rápida de la función de recompensa"""
    print("\n⚡ ENTRENAMIENTO CORTO Q-LEARNING (validación de recompensa)")
    # Configuración reducida
    configs = [
        {'episodes': 1000, 'alpha': 0.1, 'gamma': 0.99, 'epsilon': 1.0},
        {'episodes': 1000, 'alpha': 0.05, 'gamma': 0.99, 'epsilon': 0.5},
    ]
    env = descent_env.DescentEnv()
    n_actions = 7
    state_bins = [6, 6, 8, 8]
    ...
    for i, config in enumerate(configs):
        ...
        print(f"Iteración {i+1}: recompensa promedio = {np.mean(all_episode_rewards[-config['episodes']:]):.2f}")
    # Guardar modelo corto
    import pickle, os
    os.makedirs("models", exist_ok=True)
    model_path = "models/q_table_short_test.pkl"
    with open(model_path, 'wb') as f:
        pickle.dump({
            'q_table': dict(agent.q_table),
            'state_bins': agent.state_bins,
    ...
    print(f"\n✅ Modelo corto guardado en: {model_path}")
    env.close()

if __name__ == "__main__":
    train_short_qlearning() 